...
export async function GET(request: Request, { params }: { params: { period: string } }) {
  try {
    ...
    const period = params.period || "month"; // day, week, month
    ...
    breaks.forEach((brk: any) => {
      if (brk.timeSession) {
        const session = timeSessions.find((s: any) => s._id.toString() === brk.timeSession.toString());
        if (session) {
          const clockInDate = new Date(session.clockIn);
          const dateStr = clockInDate.toISOString().split("T")[0];
          const current = statsMap.get(dateStr) || { worked: 0, breaks: 0 };
          ...
          if (brk.breakStart && brk.breakEnd) {
            const duration = (new Date(brk.breakEnd).getTime() - new Date(brk.breakStart).getTime()) / 1000 / 60;
